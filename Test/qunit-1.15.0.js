	...
	urlConfig: [
		{
			id: "noglobals",
			label: "全局检测",
			tooltip: "启用这将测试如果任何测试引入了`window`对象的新属性。存储为查询字符串."
		},
		{
			id: "notrycatch",
			label: "没有 try-catch 捕获的",
			tooltip: "启用此将运行一个try-catch块之外的测试。使调试例外IE合理。存储为查询字符串。"
		}
	],
...
function toolbarLabel() {
	...
	label.setAttribute( "title", "仅显示失败的断言条目" );
	label.innerHTML = "隐藏通过断言的条目";
...
QUnit.done(function( details ) {
	var i, key,
		...
		html = [
			...
			"<span class='passed'>",
			details.passed,
			"</span> 个通过 <span class='total'>",
			details.total,
			"</span> 条断言, <span class='failed'>",
			details.failed,
			"</span> 个失败."
		].join( "" );
